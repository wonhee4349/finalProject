...
<div class="homework_wrap mb-4">
	<div class="my_profile_wrap02"> 
		<div>
			...
			<div class="profile_img"><img src="${pageContext.request.contextPath }/staff/profileImage/${studentInfo.stdntNo}"></div>         						
			<table class="table_style02 table_wd70 table_center ft_left">
				...
				<tr>
					<th>학생명</th><td>${studentInfo.stdntNm}</td>
				</tr>
				<tr>
					<th>단과대</th><td>${studentRegister.subject.college.clgNm}</td>
				</tr>
				<tr>
					<th>전공</th><td>${studentRegister.sknrgSttusMajor1}</td>
				</tr>
				<tr>
			...
			<div id="tab-1" class="tab-content current">
				<div class="pro_table">
					...
					<table class="table_style02 table_center">
						...
						<tr><th>생년월일</th><td>${studentInfo.stdntIhidnum1}</td></tr>
						<tr><th>기본주소</th><td>${studentInfo.stdntAdres1}</td></tr>
						<tr><th>상세주소</th><td>${studentInfo.stdntAdres2}</td></tr>
						<tr><th>연락처</th><td>${studentInfo.stdntTelno}</td></tr>
						<tr><th>국적</th><td>${studentInfo.nltySe}</td></tr>
						<tr><th>성별</th><td>${studentInfo.sexdstnSe}</td></tr>
						<tr><th>입학일</th><td>${studentInfo.sknrgSttusEntsch}</td></tr>
						<tr><th>입학구분</th><td>${studentInfo.sknrgSttusEnterenceSe}</td></tr>
						<tr><th>은행명</th><td>${studentInfo.bankSe}</td></tr>
						<tr><th>계좌번호</th><td>${studentInfo.stdntAcnutno}</td></tr>  				
					</table>
				</div>
			</div>
			...
			<div id="tab-2" class="tab-content">
				<div class="pro_table">
					...
					<table class="table_style02 table_center">
						...
						<c:if test="${empty studentScholarship.scholarship}">
							<tr>
								<td colspan="6">장학 목록 없음</td>
							</tr>
						</c:if>
						<c:if test="${not empty studentScholarship.scholarship}">
			...
			<div id="tab-3" class="tab-content">
				<div class="pro_table">
					...
					<table class="table_style02 table_center">
						...
						<c:if test="${empty studentScholarship.tuition}">
							<tr>
								<td colspan="8">등록금 목록 없음</td>
							</tr>
						</c:if>
						<c:if test="${not empty studentTuition.tuition}">
							<c:forEach items="${studentTuition.tuition}" var="tuition">
								<tr>
									<td>${tuition.rnum}</td>
			...
			<div id="tab-4" class="tab-content">
				<div class="pro_table">
					...
					<table class="table_style02 table_center">
						...
						<c:if test="${empty studentClub.club}">
							<tr>
								<td colspan="5">동아리 목록 없음</td>
							</tr>
						</c:if>
						<c:if test="${not empty studentClub.club}">
							<c:forEach items="${studentClub.club}" var="club">
								<tr>
									<td>${club.rnum}</td>
									<td>${club.clubSe}</td>
									<td>${club.clubNm}</td>
